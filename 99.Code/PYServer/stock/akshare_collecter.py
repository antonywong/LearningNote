...
def collect(codes: List[str], scale: int, datalen: int):
    for i, code in enumerate(codes):
        ...
        sql = ["DELETE FROM K WHERE type=%s AND code='%s' AND day=CONVERT(DATETIME,'%s',102)" % (scale, code, row["day"]) for i, row in ks.iterrows()]
        sql.extend(["INSERT INTO K (type,code,day,[open],high,low,[close],volume) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s')"
